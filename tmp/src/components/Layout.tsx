...
export default function Layout() {
  ...
  return (
    <div className={cx("min-h-screen flex selection:bg-chess-gold/30 selection:text-white", currentBgClass)}>
      ...
      <aside className="w-20 md:w-72 bg-black/40 backdrop-blur-3xl border-r border-white/[0.05] flex flex-col justify-between shrink-0 relative z-50">
      ...
      <main className="flex-1 flex flex-col relative overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto relative z-10 custom-scrollbar overscroll-none flex flex-col">
          ...
          <header className="shrink-0 pt-6 pb-2 px-6 md:px-12 flex items-center justify-between relative z-20">
             <div className={cx("flex-1 max-w-sm hidden md:flex items-center transition-all duration-700 ease-in-out", isGameRoute ? "opacity-20 hover:opacity-100 focus-within:opacity-100 grayscale hover:grayscale-0 focus-within:grayscale-0" : "opacity-100")}>
                <div className="w-full flex items-center gap-3 bg-white/[0.03] border border-white/5 rounded-full px-5 py-3 focus-within:bg-white/[0.06] focus-within:border-white/10 transition-colors shadow-inner">
                   <Search size={16} className="text-neutral-500" />
                   <input type="text" placeholder={t('topbar.search')} className="bg-transparent border-none outline-none text-sm font-black text-white placeholder:text-neutral-600 italic tracking-wide w-full" />
                </div>
             </div>
             <div className="flex flex-1 md:flex-none justify-end items-center gap-5">
                <button className="relative w-11 h-11 flex items-center justify-center bg-white/[0.03] border border-white/5 rounded-full hover:bg-white/[0.08] transition-colors group group-active:scale-95 shadow-inner">
                   <Bell size={18} className="text-neutral-400 group-hover:text-white transition-colors" />
                   <div className="absolute top-2.5 right-2.5 w-2 h-2 bg-chess-active rounded-full shadow-[0_0_8px_#00ced1] animate-pulse" />
                </button>
                <div className="h-6 w-px bg-white/5 hidden md:block" />
                <button className="flex items-center gap-4 group">
                   <div className="hidden md:flex flex-col items-end">
                      <span className="text-[0.8rem] font-black text-white tracking-tight italic">{profile?.username || user?.user_metadata?.username || 'Guest'}</span>
                      <span className="text-[0.55rem] font-black text-chess-gold uppercase tracking-[0.2em]">{profile?.role || 'PRO'}</span>
                   </div>
                   <div className="w-11 h-11 rounded-full bg-gradient-to-br from-neutral-800 to-black border border-white/10 flex items-center justify-center overflow-hidden shadow-xl ring-2 ring-transparent group-hover:ring-white/10 transition-all">
