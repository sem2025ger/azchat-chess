...
export default function HomeScreen() {
  ...
  return (
    <div className="flex flex-col h-full w-full overflow-hidden items-center justify-center bg-[#161512] transition-all relative">
      ...
      <div className="w-full max-w-[85rem] flex flex-col xl:flex-row items-center justify-center gap-8 xl:gap-20 z-10 h-full px-5 sm:px-8 md:px-12 xl:px-16 py-6 max-h-[900px]">
        ...
        <div className="hidden lg:flex flex-col items-center xl:items-end w-full xl:w-[48%] max-w-[500px] shrink-0 relative perspective-1000 mt-[5vh] xl:mt-0">
          ...
          <div className="mt-6 bg-black/70 backdrop-blur-2xl border border-white/[0.07] px-5 py-3 rounded-2xl shadow-2xl flex items-center gap-3.5 animate-bounce-slow ring-1 ring-white/[0.04] z-20 self-center xl:self-end xl:-mr-6">
        ...
        <div className="flex-1 w-full flex flex-col justify-start content-start max-w-2xl xl:max-w-[580px] z-20 xl:-mt-2 gap-5">

          {/* 1. Header & CTA */}
          <div className="flex flex-col gap-4">
            <div className="flex flex-col items-start gap-2.5">
              <div className="inline-flex items-center gap-2.5 px-3.5 py-1.5 bg-white/[0.04] border border-white/[0.07] rounded-full backdrop-blur-md">
              ...
              <h1 className="text-4xl md:text-5xl lg:text-[3.5rem] font-black text-white tracking-tighter leading-[0.95] transform-gpu">
              ...
              <p className="text-[0.82rem] text-neutral-500 font-medium leading-relaxed max-w-sm">{t('home.subtitle')}</p>
            </div>

            <Link to="/play" className="group flex items-center justify-center gap-2.5 px-8 py-3.5 bg-chess-active text-black font-black text-[0.95rem] rounded-2xl shadow-[0_20px_50px_-15px_rgba(0,206,209,0.5)] hover:scale-[1.03] hover:bg-cyan-300 transition-all duration-300 active:scale-95 w-full sm:w-auto sm:self-start">
          ...
          {/* 2. Mini Stats */}
          <div className="flex items-center justify-between px-6 py-3.5 bg-white/[0.03] backdrop-blur-md rounded-2xl border border-white/[0.06]">
            {stats.map((s, i) => (
          ...
          <div className="grid grid-cols-2 gap-2.5">
            {/* Continue Last Game */}
            <Link to="/game" className="flex items-center gap-3 bg-white/[0.02] border border-white/[0.05] p-3 rounded-2xl hover:bg-white/[0.05] hover:border-white/[0.12] transition-all duration-300 group">
              <div className="w-11 h-11 rounded-xl bg-chess-active/10 border border-chess-active/20 flex flex-col items-center justify-center shrink-0 group-hover:scale-105 transition-transform duration-300">
              ...
              <div className="flex flex-col overflow-hidden min-w-0">
                <span className="text-[0.58rem] font-bold text-neutral-600 uppercase tracking-[0.15em]">{t('home.dashboard.continue')}</span>
                <span className="text-[0.78rem] font-black text-white truncate group-hover:text-chess-active transition-colors">{profile?.username || user?.user_metadata?.username || 'Guest'}</span>
              </div>
            </Link>

            {/* Last Analysis */}
            <Link to="/game" className="flex items-center gap-3 bg-white/[0.02] border border-white/[0.05] p-3 rounded-2xl hover:bg-white/[0.05] hover:border-white/[0.12] transition-all duration-300 group">
              <div className="w-11 h-11 rounded-xl bg-chess-gold/10 border border-chess-gold/20 flex flex-col items-center justify-center shrink-0 group-hover:scale-105 transition-transform duration-300">
              ...
              <div className="flex flex-col overflow-hidden min-w-0">
                <span className="text-[0.58rem] font-bold text-neutral-600 uppercase tracking-[0.15em]">{t('home.dashboard.analysis')}</span>
                <span className="text-[0.78rem] font-black text-white truncate group-hover:text-chess-gold transition-colors">Sicilian Defense</span>
              </div>
            </Link>

            {/* Community Activity */}
            <Link to="/chat" className="flex items-center gap-3 bg-white/[0.02] border border-white/[0.05] p-3 rounded-2xl hover:bg-white/[0.05] hover:border-white/[0.12] transition-all duration-300 group">
              <div className="w-11 h-11 rounded-xl bg-emerald-500/10 border border-emerald-500/20 flex flex-col items-center justify-center shrink-0 group-hover:scale-105 transition-transform duration-300">
                <div className="w-2.5 h-2.5 bg-emerald-500 rounded-full animate-pulse shadow-[0_0_10px_#10b981]" />
              </div>
              <div className="flex flex-col overflow-hidden min-w-0">
                <span className="text-[0.58rem] font-bold text-neutral-600 uppercase tracking-[0.15em]">{t('home.dashboard.community')}</span>
                <span className="text-[0.78rem] font-black text-white truncate group-hover:text-emerald-400 transition-colors">4 {t('play.friend')} Online</span>
              </div>
            </Link>

            {/* Daily Tournament */}
            <Link to="/play" className="flex items-center gap-3 bg-white/[0.02] border border-white/[0.05] p-3 rounded-2xl hover:bg-white/[0.05] hover:border-white/[0.12] transition-all duration-300 group">
              <div className="w-11 h-11 rounded-xl bg-purple-500/10 border border-purple-500/20 flex flex-col items-center justify-center shrink-0 group-hover:scale-105 transition-transform duration-300">
              ...
              <div className="flex flex-col overflow-hidden min-w-0">
                <span className="text-[0.58rem] font-bold text-neutral-600 uppercase tracking-[0.15em]">{t('home.dashboard.tournament')}</span>
                <span className="text-[0.78rem] font-black text-white truncate group-hover:text-purple-400 transition-colors">Global Arena Blitz</span>
              </div>
            </Link>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2.5">
            <FeatureCard
              to="/play" icon={<Play size={17} />} title={t('home.cards.play.title')}
